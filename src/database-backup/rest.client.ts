...
import { ErrorResponse, TriggerDatabaseBackupRequest, TriggerDatabaseBackupResponse } from './rest.zod.js'
...
export class MambuDatabaseBackup {
    ...
    public downloadBackup({
        path,
        auth = [['apiKey'], ['basic']],
    }: { path: { databaseBackupVersion: string }; auth?: string[][] | string[] }): Promise<
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).get(`database/backup/${path.databaseBackupVersion}`, {}),
            {
    ...
    public triggerBackup({
        ...
        headers,
        auth = [['apiKey'], ['basic']],
    }: { body: TriggerDatabaseBackupRequest; headers?: { 'Idempotency-Key'?: string }; auth?: string[][] | string[] }): Promise<
