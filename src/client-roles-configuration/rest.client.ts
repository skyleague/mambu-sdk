...
/**
 * configuration/clientroles.yaml
 */
export class MambuClientRolesConfiguration {
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).get('configuration/clientroles.yaml', {
                searchParams: query ?? {},
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).put('configuration/clientroles.yaml', {}),
            {
                200: { safeParse: (x: unknown) => ({ success: true, data: x }) },
