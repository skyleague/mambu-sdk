...
/**
 * configuration/depositproducts.yaml
 */
export class MambuDepositProductsConfiguration {
    ...
    public get({
        query,
        auth = [['apiKey'], ['basic']],
    }: { query?: { type?: string }; auth?: string[][] | string[] } = {}): Promise<
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).get('configuration/depositproducts.yaml', {
                searchParams: query ?? {},
    ...
    public update({
        headers,
        auth = [['apiKey'], ['basic']],
    }: { headers?: { 'X-Mambu-Async'?: string; 'X-Mambu-Callback'?: string }; auth?: string[][] | string[] } = {}): Promise<
        | SuccessResponse<'200', unknown>
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).put('configuration/depositproducts.yaml', {
                headers: headers ?? {},
            }),
            {
                200: { safeParse: (x: unknown) => ({ success: true, data: x }) },
