...
/**
 * configuration/holidays.yaml
 */
export class MambuHolidaysConfiguration {
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).get('configuration/holidays.yaml', {
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).put('configuration/holidays.yaml', {}),
            {
                200: { safeParse: (x: unknown) => ({ success: true, data: x }) },
                400: { safeParse: (x: unknown) => ({ success: true, data: x }) },
                401: { safeParse: (x: unknown) => ({ success: true, data: x }) },
