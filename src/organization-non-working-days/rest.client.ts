...
import { ErrorResponse, NonWorkingDays } from './rest.zod.js'

/**
 * organization/holidays/nonworkingdays
 */
export class MambuOrganizationNonWorkingDays {
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).get('organization/holidays/nonworkingdays', {
    ...
    public updateNonWorkingDays({
        body,
        auth = [['apiKey'], ['basic']],
    }: { body: NonWorkingDays; auth?: string[][] | string[] }): Promise<
        | SuccessResponse<'200', NonWorkingDays>
        | FailureResponse<'400', ErrorResponse, 'response:statuscode'>
        | FailureResponse<'401', ErrorResponse, 'response:statuscode'>
        | FailureResponse<'403', ErrorResponse, 'response:statuscode'>
    ...
    > {
        ...
        return this.awaitResponse(
            this.buildClient(auth).put('organization/holidays/nonworkingdays', {
                json: _body.right,
                headers: { Accept: 'application/vnd.mambu.v2+json' },
            }),
            {
