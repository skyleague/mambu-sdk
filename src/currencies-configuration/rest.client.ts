...
export class MambuCurrenciesConfiguration {
    ...
    public get({
        query,
        auth = [['apiKey'], ['basic']],
    }: { query?: { startDate?: string }; auth?: string[][] | string[] } = {}): Promise<
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).get('configuration/currencies.yaml', {
                searchParams: query ?? {},
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).put('configuration/currencies.yaml', {}),
            {
                200: { safeParse: (x: unknown) => ({ success: true, data: x }) },
