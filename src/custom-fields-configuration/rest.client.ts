 ...
 */
export class MambuCustomFieldsConfiguration {
    ...
    public get({
        query,
        auth = [['apiKey'], ['basic']],
    }: { query?: { availableFor?: string }; auth?: string[][] | string[] } = {}): Promise<
    ...
    > {
        return this.awaitResponse(
            this.buildClient(auth).put('configuration/customfields.yaml', {
                headers: headers ?? {},
            }),
            {
                200: { safeParse: (x: unknown) => ({ success: true, data: x }) },
